...
$statement = $conn->prepare("SELECT numero_contrato,fecha_entrega,nombre_entregable,cantidad_entregable,direccion_entregable,descripcion,((porcentaje*precio_unitario)*cantidad_entregable)*(DATEDIFF(e.fecha_entrega,em.fecha_maxima)) AS penalizacion FROM entregables AS e INNER JOIN contrato AS c ON c.id_contrato= e.id_contrato INNER JOIN entregas_m AS em ON c.id_contrato = em.id_contrato WHERE c.numero_contrato =?");
