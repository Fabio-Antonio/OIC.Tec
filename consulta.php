<?php
   ...
   require_once("url.php");
    ...
    $statement = $conn->prepare("SELECT numero_contrato,contrato_compranet,nombre_unidad_compradora,numero_unidad,unidad, clave_requirente,monto_max,monto_min,objeto_contratacion,procedimientos,suficiencia,inicio_vigencia,fin_vigencia,notificacion_adjudicada,formalizacion_contrato,requisicion_contrato ,sat,imss,infonavit,garantia_cumplimiento,licitacion FROM contrato INNER JOIN unidad_compradora ON contrato.id_unidad_compradora=unidad_compradora.id_unidad_compradora INNER JOIN unidad_requirente ON contrato.id_unidad_requirente=unidad_requirente. id_requirente INNER JOIN procedimientos_contratacion ON contrato.id_procedimiento_contratacion=procedimientos_contratacion.id_procedimiento_contratacion INNER JOIN contrato_fechas ON contrato.id_contrato =contrato_fechas.id_contrato LEFT JOIN consolidado ON contrato.id_consolidado= consolidado.id_consolidado WHERE numero_contrato= ?");
...
&requisicion_contrato=$requisicion_contrato&sat=$sat&imss=$imss&infonavit=$infonavit&garantia_cumplimiento=$garantia_cumplimiento&flag=$valor&licitacion=$licitacion");
